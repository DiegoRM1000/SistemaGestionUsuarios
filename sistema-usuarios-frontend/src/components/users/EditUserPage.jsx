...
const EditUserPage = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                // CORRECCIÓN CLAVE: Obtener el usuario por su ID y los roles desde el endpoint correcto
                const [userResponse, rolesResponse] = await Promise.all([
                    apiClient.get(`/users/${id}`),
                    apiClient.get('/roles/all') // CORRECCIÓN: Endpoint para obtener roles
                ]);
                ...
                navigate('/dashboard/users'); // CORRECCIÓN: La ruta para el listado es /dashboard/users
            } finally {
                setLoading(false);
            }
        };
